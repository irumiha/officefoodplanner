...
private object TableRepositorySQL {
  ...
  def updateMany[E: DoobieColumnList : DoobieIDColumn : Write](table: Table): Update[E] = {
  ...
  def deleteManyIDs[E: DoobieIDColumn](table: Table, rows: List[UUID]): Update0 = {
...
abstract class TableRepository[E: Encoder : Decoder : UuidKeyEntity : DoobieColumnList : DoobieIDColumn: DoobieColumnValues: Read: Write]
  ...
  override def update(entities: List[E]): ConnectionIO[Int] =
    updateMany(table).updateMany(entities)
